...
# Call the function
welcome_to_my_universe(
  your_name = "Yevheniia",
  universe_name = "Bernolákovo Mini-Universe"
)
...
# Load your dataset
bern_data <- load_and_inspect("bernolakovo_df.csv")
...
```{r}
filter_by_density <- function(data, min_density) {
  
  result <- data %>%
    filter(!is.na(dist_km), dist_km > 0) %>%
    mutate(density = pop / dist_km) %>%
    filter(density >= min_density) %>%
    select(district, zone, density) %>%
    arrange(desc(density))
...
```{r}
# Example call: districts with density >= 500
density_result <- filter_by_density(bern_data, min_density = 500)
...
generate_report <- function(data, zone_name) {
  ...
  # 1. Filter for the selected zone
  zone_data <- data %>%
    filter(zone == zone_name)
  
  # If there are no matching rows
  if (nrow(zone_data) == 0) {
    cat("No records found for zone:", zone_name, "\n\n")
    return(invisible(NULL))
  }
  
  # 2. Calculate key statistics
  avg_population  <- mean(zone_data$pop, na.rm = TRUE)
  max_population  <- max(zone_data$pop, na.rm = TRUE)
  min_population  <- min(zone_data$pop, na.rm = TRUE)
  avg_distance    <- mean(zone_data$dist_km, na.rm = TRUE)
  count_records   <- nrow(zone_data)
  share_playground <- mean(zone_data$has_playground, na.rm = TRUE) * 100
  
  # 3. Print formatted report
  cat("----------------------------------------\n")
  cat("Report for zone:", zone_name, "\n")
  cat("----------------------------------------\n")
  cat("Number of districts:", count_records, "\n")
  cat("Average population:", round(avg_population, 1), "\n")
  cat("Maximum population:", max_population, "\n")
  cat("Minimum population:", min_population, "\n")
  cat("Average distance (km):", round(avg_distance, 2), "\n")
  cat("Playground availability (%):", round(share_playground, 1), "\n")
  cat("----------------------------------------\n\n")
}
...
```{r}
generate_report(bern_data, "residential")
generate_report(bern_data, "mixed")
...
hist(
  bern_data$pop,
  main = "Population Distribution in Bernolákovo Districts",
...
population_distance_plot <- ggplot(
  data = bern_data,
  aes(x = dist_km, y = pop)
) +
  ...
  labs(
    title    = "Population vs Distance from Centre",
    subtitle = "Each point represents one district",
    x        = "Distance from centre (km)",
    y        = "Population (number of residents)"
  ) +
  theme_minimal()

population_distance_plot
...
```{r}
ggplot(data = bern_data, aes(x = zone, y = pop, fill = zone)) +
  geom_boxplot() +
  labs(
    title = "Comparison of Population by Zone Type",
    x     = "Zone Type",
    y     = "Population (number of residents)"
  ) +
  theme_light() +
...
```{r}
ggplot(data = bern_data, aes(x = pop)) +
  geom_histogram(bins = 10, fill = "coral", color = "black") +
  facet_wrap(~ zone) +
  labs(
    title = "Population Distribution Across Zone Types",
    x     = "Population (number of residents)",
...
```{r}
ggsave(
  "my_universe_plot.png",
  plot   = population_distance_plot,
  width  = 8,
  height = 6
)
```
